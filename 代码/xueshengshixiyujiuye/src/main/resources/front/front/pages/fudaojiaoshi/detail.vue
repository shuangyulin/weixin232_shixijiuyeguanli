...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.fudaojiaoshiName}}
						</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('fudaojiaoshi', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.fudaojiaoshiPhoto ? this.detail.fudaojiaoshiPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('fudaojiaoshiLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					fudaojiaoshiId: this.id
				});
				// 如果是第一页数据置空
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../fudaojiaoshiLiuyan/add-or-update?fudaojiaoshiId=${this.id}`)
            },
		}
	}
</script>
